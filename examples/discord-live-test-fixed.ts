...
async function runLiveTest() {
  ...
  // Connect to the AXON server - this will load the component
  await discord.connect(discordUrl);
  ...
  setInterval(() => {
    ...
    events.forEach((facet, id) => {
      if (facet.type === 'event' && id.startsWith('discord:message:')) {
        // Process new messages that haven't been handled
        if (!facet.attributes?.processed) {
          handleDiscordMessage({ 
            topic: 'discord:message', 
            payload: facet.content 
          });
          // Mark as processed
          space.activateAgent('discord', {
            reason: 'mark-processed',
            metadata: { facetId: id }
          });
        }
      }
